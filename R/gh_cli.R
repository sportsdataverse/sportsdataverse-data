...
#' @export
gh_cli_release_tags <- function(repo = "sportsdataverse/sportsdataverse-data") {
...
#' @import data.table
#' @export
gh_cli_release_assets <- function(
  tag,
  ...,
  repo = "sportsdataverse/sportsdataverse-data"
) {
...
gh_cli_rate_limits <- function(verbose = TRUE) {
  ...
  rate$reset_parsed <- format(
    as.POSIXct(rate$reset, origin = "1970-01-01", tz = "UTC"),
    usetz = TRUE
  )

  if (isTRUE(verbose)) {
    cli::cli_ul(paste0(names(rate), " : ", rate))
  }
